...
At this time, the BCIP team does not accept unsolicited code contributions.
...
The BCIP team may invite an external contributor to submit a pull request when:
...
Pull requests that have not been explicitly invited by a member of the BCIP team will be closed without review.

**Why we do not generally accept external code contributions**

In the past, the BCIP team accepted external pull requests for bug fixes. While we appreciated the effort and engagement from the community, this model did not scale well.
...
If you are invited by a BCIP team member to contribute a PR, here is the recommended development workflow.
...
3. **Document behavior.** If your change affects user-facing behavior, update the README, inline help (`bcip --help`), or relevant example projects.
...
Together we can make BCIP Agent an incredible tool. **Happy hacking!** :rocket:
