...
pub enum Command {
    /// Submit a new BCIP Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a BCIP Cloud task.
    Status(StatusCommand),
    /// List BCIP Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a BCIP Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a BCIP Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in BCIP Cloud.
    ...
    /// Git branch to run in BCIP Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// BCIP Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// BCIP Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// BCIP Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
