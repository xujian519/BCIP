...
async fn model_migration_prompt_only_shows_for_deprecated_models() {
    ...
    let glm = presets
        .iter_mut()
        .find(|preset| preset.model == "glm-5.1")
        .expect("glm-5.1 preset present");
    glm.upgrade = Some(ModelUpgrade {
...
fn select_model_availability_nux_skips_missing_and_exhausted_models() {
    ...
    let first_model = presets
        .iter_mut()
        .find(|preset| preset.model == "deepseek-v4-pro")
        .expect("deepseek-v4-pro preset present");
    first_model.availability_nux = Some(ModelAvailabilityNux {
        message: "deepseek-v4-pro is available".to_string(),
    });
    let second_model = presets
        .iter_mut()
        .find(|preset| preset.model == "deepseek-v4-flash")
        .expect("deepseek-v4-flash preset present");
    second_model.availability_nux = Some(ModelAvailabilityNux {
...
async fn accepted_model_migration_persists_target_default_reasoning_effort() {
    ...
    assert_matches!(
        ...
        AppEvent::PersistModelMigrationPromptAcknowledged { from_model, to_model }
            if from_model == "glm-5.1" && to_model == "deepseek-v4-pro"
    );
...
async fn model_migration_prompt_respects_hide_flag_and_self_target() {
    ...
    seen.insert("glm-5.1".to_string(), "deepseek-v4-pro".to_string());
    assert!(!should_show_model_migration_prompt(
        "glm-5.1",
        "deepseek-v4-pro",
        &seen,
...
async fn model_migration_prompt_skips_when_target_missing_or_hidden() {
    ...
    available.retain(|preset| preset.model != "glm-5.1");
...
async fn model_migration_prompt_shows_for_hidden_model() {
    ...
    let current = available_models
        .iter_mut()
        .find(|preset| preset.model == "kimi-for-coding")
        .expect("kimi-for-coding preset present");
    ...
    assert!(
        ...
        "expected kimi-for-coding to be hidden from picker for this test"
    );
