...
//!
//! - `parse`：解析 OA 文本，提取类型、引用文献、影响的权利要求及审查员意见。
//! - `analyze_and_recommend`：根据 OA 类型推荐答复策略（争辩、修改权利要求、混合策略）。
...
static RE_CITATION: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?:CN|US|WO|EP|JP|KR)\d{6,}[A-Z]?").expect("invalid regex: RE_CITATION")
});
static RE_CLAIM_NUMBER: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"权利要求\s*(\d+)").expect("invalid regex: RE_CLAIM_NUMBER"));
static RE_CLAIM_RANGE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"第\s*(\d+)\s*[-至到]\s*(\d+)\s*项").expect("invalid regex: RE_CLAIM_RANGE")
});

static RE_EXAMINER_ARGS: [LazyLock<Regex>; 3] = [
    LazyLock::new(|| {
        Regex::new(r"(?s)审查意见[：:](.+?)(?:\n答复|$)")
            .expect("invalid regex: RE_EXAMINER_ARGS[0]")
    }),
    LazyLock::new(|| {
        Regex::new(r"(?s)驳回理由[：:](.+?)(?:\n答复|$)")
            .expect("invalid regex: RE_EXAMINER_ARGS[1]")
    }),
    LazyLock::new(|| {
        Regex::new(r"(?s)认为[：:](.+?)(?:\n|$)").expect("invalid regex: RE_EXAMINER_ARGS[2]")
    }),
];

/// 解析 OA 文本，返回结构化 OfficeAction
pub fn parse(text: &str) -> OfficeAction {
    let oa_type = detect_oa_type(text);
    let citations = extract_citations(text);
    let affected_claims = extract_affected_claims(text);
    let examiner_arguments = extract_examiner_arguments(text);
    OfficeAction {
        oa_type,
        citations,
        examiner_arguments,
    ...
    match &oa.oa_type {
        ...
        OaType::InventiveStep => {
            strategies.push(ResponseStrategy {
                ...
                reasoning: "创造性争辩需证明区别特征非显而易见，同时修改权利要求增加限定".into(),
                confidence: 0.5,
            });
            strategies.push(ResponseStrategy {
                strategy_type: ResponseStrategyType::AmendClaims,
