...
//!
//! 提供两套能力：
//! - `parse`：将专利说明书原文按标准章节（技术领域、背景技术等）拆解
//! - `extract_features`：从说明书文本中提取结构化技术特征
...
static RE_INVENTION_NAME: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?:发明名称|名称)[：:]\s*(.+?)(?:\n|$)").expect("invalid regex: RE_INVENTION_NAME")
});
static RE_TECH_FIELD: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?s)(?:技术领域)[：:]\s*(.+?)(?:\n(?:背景技术|发明内容|现有技术)|$)")
        .expect("invalid regex: RE_TECH_FIELD")
});
static RE_BACKGROUND: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?s)(?:背景技术|现有技术)[：:]\s*(.+?)(?:\n(?:发明内容|技术问题|技术方案)|$)")
        .expect("invalid regex: RE_BACKGROUND")
});
static RE_INVENTION_CONTENT: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?s)(?:发明内容)[：:]\s*(.+?)(?:\n(?:具体实施方式|附图说明)|$)")
        .expect("invalid regex: RE_INVENTION_CONTENT")
});
static RE_TECH_PROBLEM: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?s)(?:技术问题|所要解决的技术问题)[：:]\s*(.+?)(?:\n|$)")
        .expect("invalid regex: RE_TECH_PROBLEM")
});
static RE_TECH_SOLUTION: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?s)(?:技术方案|技术解决方案)[：:]\s*(.+?)(?:\n|$)")
        .expect("invalid regex: RE_TECH_SOLUTION")
});
static RE_TECH_EFFECT: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?s)(?:技术效果|有益效果)[：:]\s*(.+?)(?:\n|$)")
        .expect("invalid regex: RE_TECH_EFFECT")
});
static RE_EMBODIMENT: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?s)(?:具体实施方式|实施例)[：:]\s*(.+)$").expect("invalid regex: RE_EMBODIMENT")
});
static RE_DRAWING_DESC: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?s)(?:附图说明)[：:]\s*(.+?)(?:\n(?:具体实施方式|发明内容)|$)")
        .expect("invalid regex: RE_DRAWING_DESC")
});
...
static RE_FALLBACK_FIELD: [LazyLock<Regex>; 2] = [
    LazyLock::new(|| {
        Regex::new(r"(?:所属)?技术领域[：:][^\n]*").expect("invalid regex: RE_FALLBACK_FIELD[0]")
    }),
    LazyLock::new(|| Regex::new(r"本发明涉及[^\n]*").expect("invalid regex: RE_FALLBACK_FIELD[1]")),
];
static RE_FALLBACK_PROBLEM: [LazyLock<Regex>; 2] = [
    LazyLock::new(|| {
        Regex::new(r"(?:所要解决)?技术问题[：:][^\n]*")
            .expect("invalid regex: RE_FALLBACK_PROBLEM[0]")
    }),
    LazyLock::new(|| Regex::new(r"为了解决[^\n]*").expect("invalid regex: RE_FALLBACK_PROBLEM[1]")),
];
static RE_FALLBACK_SOLUTION: [LazyLock<Regex>; 2] = [
    LazyLock::new(|| {
        Regex::new(r"技术方案[：:][^\n]*").expect("invalid regex: RE_FALLBACK_SOLUTION[0]")
    }),
    LazyLock::new(|| Regex::new(r"采用[^\n]*").expect("invalid regex: RE_FALLBACK_SOLUTION[1]")),
];

// 特征提取正则
static RE_COMPONENT: [LazyLock<Regex>; 2] = [
    LazyLock::new(|| {
        Regex::new(r"([\w\u4e00-\u9fff]{1,8})(?:层|模块|单元|部件|装置|器件)")
            .expect("invalid regex: RE_COMPONENT[0]")
    }),
    LazyLock::new(|| {
        Regex::new(r"([\w\u4e00-\u9fff]{1,8})(?:器|机|设备)")
            .expect("invalid regex: RE_COMPONENT[1]")
    }),
];
static RE_STEP: [LazyLock<Regex>; 2] = [
    LazyLock::new(|| {
        Regex::new(r"步骤[一二三四五六七八九十\d]+[：:]\s*([^\n，。；]+)")
            .expect("invalid regex: RE_STEP[0]")
    }),
    LazyLock::new(|| {
        Regex::new(r"第[一二三四五六七八九十\d]+步[：:]\s*([^\n，。；]+)")
            .expect("invalid regex: RE_STEP[1]")
    }),
];
static RE_PARAMETER: [LazyLock<Regex>; 2] = [
    LazyLock::new(|| {
        Regex::new(r"([^\s，。]{2,10})(?:大小|数量|长度|宽度|厚度|重量)[：:]\s*([^\n，。]+)")
            .expect("invalid regex: RE_PARAMETER[0]")
    }),
    LazyLock::new(|| {
        Regex::new(r"([^\s，。]{2,10})为\s*([^\n，。]+?)(?:，|。|$)")
            .expect("invalid regex: RE_PARAMETER[1]")
    }),
];
...
static RE_SOLUTION_SECTION: [LazyLock<Regex>; 2] = [
    LazyLock::new(|| {
        Regex::new(r"(?s)技术方案[：:](.+?)(?:\n技术效果|\n具体实施方式|$)")
            .expect("invalid regex: RE_SOLUTION_SECTION[0]")
    }),
    LazyLock::new(|| {
        Regex::new(r"(?s)采用[^\n]*?(?:\n技术效果|\n具体实施方式|$)")
            .expect("invalid regex: RE_SOLUTION_SECTION[1]")
    }),
];
static RE_PROBLEM_SECTION: [LazyLock<Regex>; 2] = [
    LazyLock::new(|| {
        Regex::new(r"(?s)技术问题[：:](.+?)(?:\n技术方案|$)")
            .expect("invalid regex: RE_PROBLEM_SECTION[0]")
    }),
    LazyLock::new(|| {
        Regex::new(r"(?s)所要解决的技术问题[：:](.+?)(?:\n技术方案|$)")
            .expect("invalid regex: RE_PROBLEM_SECTION[1]")
    }),
];
static RE_EFFECTS_SECTION: [LazyLock<Regex>; 2] = [
    LazyLock::new(|| {
        Regex::new(r"(?s)技术效果[：:](.+?)(?:\n具体实施方式|$)")
            .expect("invalid regex: RE_EFFECTS_SECTION[0]")
    }),
    LazyLock::new(|| {
        Regex::new(r"(?s)有益效果[：:](.+?)(?:\n具体实施方式|$)")
            .expect("invalid regex: RE_EFFECTS_SECTION[1]")
    }),
];
...
pub fn parse(text: &str) -> DisclosureDoc {
    ...
    let confidence = calculate_confidence(&sections, text);
    DisclosureDoc {
...
fn extract_sections(text: &str) -> HashMap<String, String> {
    ...
    if sections.get("技术领域").is_none_or(|s| s.is_empty())
        && let Some(v) = extract_by_static_patterns(text, &RE_FALLBACK_FIELD)
    {
        sections.insert("技术领域".into(), v);
    }
    if sections.get("技术问题").is_none_or(|s| s.is_empty())
        && let Some(v) = extract_by_static_patterns(text, &RE_FALLBACK_PROBLEM)
    {
        sections.insert("技术问题".into(), v);
    }
    if sections.get("技术方案").is_none_or(|s| s.is_empty())
        && let Some(v) = extract_by_static_patterns(text, &RE_FALLBACK_SOLUTION)
    {
...
pub fn extract_features(
    ...
    let solution_text = sections
        ...
        .map(String::as_str)
        .unwrap_or_else(|| extract_solution_section(text));

    let mut features = Vec::new();
    features.extend(extract_component_features(solution_text));
    features.extend(extract_step_features(solution_text));
    features.extend(extract_parameter_features(solution_text));
...
pub fn extract_problem_feature_effects(
    ...
    let problem = sections
        ...
        .cloned()
        .unwrap_or_else(|| extract_problem_section(text).to_string());
    let effects_text = sections
        .and_then(|s| s.get("技术效果"))
        .cloned()
        .unwrap_or_else(|| extract_effects_section(text).to_string());
    let effects = parse_effects(&effects_text);
...
pub(crate) fn extract_component_features(text: &str) -> Vec<TechnicalFeature> {
    let mut features = Vec::new();
        ...
        for cap in re.captures_iter(text) {
            if let Some(m) = cap.get(1) {
                ...
                features.push(TechnicalFeature {
                    ...
                    function: Some(infer_function(&c, text)),
                });
            }
        }
    }
...
pub(crate) fn extract_step_features(text: &str) -> Vec<TechnicalFeature> {
    let mut features = Vec::new();
    for re in &RE_STEP {
        for cap in re.captures_iter(text) {
            if let Some(m) = cap.get(1) {
...
如图1所示，本发明的图像识别方法采用深度卷积神经网络结构。";
    ...
    fn test_extract_features() {
        ...
        let features = extract_features("", Some(&sections));
        assert!(
            !features.is_empty(),
    ...
    fn test_extract_component_features() {
        let f = extract_component_features("包括输入层、卷积处理层、池化计算层和全连接分析层");
    ...
    fn test_extract_step_features() {
        let f =
            extract_step_features("步骤一：输入图像数据；步骤二：进行特征提取；步骤三：分类识别");
