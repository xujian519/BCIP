...
/// Determine where BCIP should store CLI auth credentials.
...
/// Determine where BCIP should store and read MCP credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
...
pub struct AnalyticsConfigToml {
    /// When `false`, disables analytics across BCIP product surfaces in this profile.
...
pub struct FeedbackConfigToml {
    /// When `false`, disables the feedback flow across BCIP product surfaces.
    pub enabled: Option<bool>,
}
...
pub struct AppConfig {
    /// When `false`, BCIP does not surface this app.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
/// (primarily the BCIP IDE extension). NOTE: these are different from
