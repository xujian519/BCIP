...
/// Minimal launcher that initializes the BCIP app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "Codex", version, about = "Bootstrap BCIP app-server", long_about = None)]
struct Cli {
...
enum CliCommand {
    ...
    /// Send a user message through the BCIP app-server.
    SendMessage {
    ...
    /// Fetch the current account rate limits from the BCIP app-server.
    GetAccountRateLimits,
    /// List the available models from the BCIP app-server.
    #[command(name = "model-list")]
    ModelList,
    /// List stored threads from the BCIP app-server.
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    ...
                    title: Some("BCIP Toy App Server".to_string()),
