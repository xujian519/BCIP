...
mod tests {
    ...
    fn subject_matter_article5_blocked() {
        ...
        let result = QualityTools::subject_matter_checker(input).unwrap();
        let blocks = result["blocking_issues"].as_array().unwrap();
        assert!(blocks.iter().any(|b| b.as_str().unwrap().contains("第5条")));
    }
    ...
    fn unified_quality_includes_vague_issues() {
        ...
        let result = QualityTools::unified_quality(input).unwrap();
        let issues = result["issues"].as_array().unwrap();
