...
use codex_patent_domain::oa::analyze_and_recommend;
use codex_patent_domain::oa::parse;
...
pub fn oa_strategist(input: OaStrategyInput) -> Result<serde_json::Value, String> {
    ...
    let strategies = analyze_and_recommend(&oa);
...
pub fn patent_responder(input: ResponderInput) -> Result<serde_json::Value, String> {
    ...
    let template = get_response_template(&oa.oa_type, best);
    Ok(serde_json::json!({
        "strategy": best,
...
pub fn strategy_argument_generator(input: ArgumentInput) -> Result<serde_json::Value, String> {
    ...
    Ok(serde_json::json!({
        ...
        "argument": build_argument(&input.oa_type, &input.differences, &input.technical_effects),
    }))
}

pub fn response_template(input: TemplateInput) -> Result<serde_json::Value, String> {
    let template = get_response_template(
        &match input.oa_type.as_str() {
            "novelty" | "新颖性" => codex_patent_core::OaType::Novelty,
            "inventive" | "创造性" => codex_patent_core::OaType::InventiveStep,
            _ => codex_patent_core::OaType::Other(input.oa_type.clone()),
    ...
    t.insert("StrategyArgumentGenerator".into(), |input| {
        Box::pin(async move {
            let parsed: ArgumentInput =
                serde_json::from_value(input).map_err(|e| format!("{e}"))?;
            strategy_argument_generator(parsed)
        })
    });
...
mod tests {
    ...
    fn oa_type_str_novelty() {
        assert_eq!(oa_type_str(&codex_patent_core::OaType::Novelty), "新颖性");
    }

    #[test]
    fn oa_type_str_inventive_step() {
        assert_eq!(
            oa_type_str(&codex_patent_core::OaType::InventiveStep),
            "创造性"
        );
    }
    ...
    fn oa_type_str_other() {
        assert_eq!(
            oa_type_str(&codex_patent_core::OaType::Other("test".into())),
            "其他"
        );
    }
    ...
    fn build_argument_novelty_with_differences() {
        let result = build_argument("novelty", &["特征A".into(), "特征B".into()], &[]);
    ...
    fn build_argument_empty_inputs_uses_defaults() {
        let result = build_argument("novelty", &[], &[]);
        assert!(result.contains("区别技术特征"));
    }

    #[test]
    fn build_argument_default_type() {
        let result = build_argument("clarity", &[], &[]);
    ...
    fn get_response_template_amend() {
        let tpl = get_response_template(&codex_patent_core::OaType::Novelty, "amend");
    ...
    fn get_response_template_argue() {
        let tpl = get_response_template(&codex_patent_core::OaType::InventiveStep, "argue");
        assert!(tpl.contains("区别技术特征"));
    }

    #[test]
    fn get_response_template_default_strategy() {
        let tpl = get_response_template(&codex_patent_core::OaType::Clarity, "hybrid");
    ...
    fn strategy_argument_generator_novelty_defaults_legal_basis() {
        ...
        let result = strategy_argument_generator(input).unwrap();
    ...
    fn response_template_novelty() {
        ...
        let result = response_template(input).unwrap();
    ...
    fn patent_responder_minimal() {
        let input = ResponderInput {
            oa_content: "审查意见：本申请不具备新颖性。对比文件CN123456A公开了全部技术特征。"
                .into(),
            strategy: None,
            patent_info: None,
        };
        let result = patent_responder(input).unwrap();
