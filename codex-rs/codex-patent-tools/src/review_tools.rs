...
#[derive(Debug, Deserialize)] pub struct FormalCheckInput { pub claims: Vec<String>, pub specification_sections: Option<Vec<String>> }
#[derive(Debug, Deserialize)] pub struct QualityAssessInput { pub claims: Vec<String>, pub specification_word_count: usize }
#[derive(Debug, Deserialize)] pub struct SubjectMatterCheckInput { pub claim_text: String }
#[derive(Debug, Deserialize)] pub struct UnityCheckInput { pub claims: Vec<String> }
#[derive(Debug, Deserialize)] pub struct OaStrategyInput { pub rejection_type: String, pub differences: Option<Vec<String>>, pub technical_effects: Option<Vec<String>>, pub prior_art_different_field: Option<bool> }
#[derive(Debug, Deserialize)] pub struct OaResponseTemplateInput { pub template_type: String, pub arguments: Option<Vec<String>> }
...
impl ReviewTools {
    ...
    pub fn quality_assess(input: QualityAssessInput) -> Result<serde_json::Value, String> {
        ...
        let length_score = if word_count > 1000 { 1.0 } else { word_count as f64 / 1000.0 };
        Ok(serde_json::json!({"overall_score": score * length_score, "claim_count": input.claims.len(), "word_count": word_count}))
    }

    pub fn subject_matter_check(input: SubjectMatterCheckInput) -> Result<serde_json::Value, String> {
        let lower = input.claim_text.to_lowercase();
        let excluded = ["智力活动", "医疗诊断", "原子核", "科学发现", "游戏规则"];
        let violations: Vec<&&str> = excluded.iter().filter(|kw| lower.contains(&kw.to_lowercase())).collect();
    ...
    pub fn unity_check(input: UnityCheckInput) -> Result<serde_json::Value, String> {
        ...
        let mut common: std::collections::HashSet<&str> = input.claims[0].split_whitespace().collect();
    ...
    pub fn oa_strategy(input: OaStrategyInput) -> Result<serde_json::Value, String> {
        ...
        let ctx = CaseContext {
            invention: None, prior_art_contains_all: None,
            differences: input.differences, technical_effect: None,
            performance_improvement: None, obviousness: None,
        ...
        let r = engine.suggest_oa_strategy(&ctx).map_err(|e| format!("{e}"))?;
    ...
    pub fn response_template(input: OaResponseTemplateInput) -> Result<serde_json::Value, String> {
        let templates = [
            ("新颖性争辩", "申请人认为，对比文件未公开权利要求的技术特征，本申请具备新颖性，符合专利法第22条第2款。"),
            ("创造性争辩", "区别技术特征具有非显而易见的技术效果，现有技术未给出技术启示，本申请具备创造性。"),
            ("修改方案", "申请人将权利要求合并，形成新的独立权利要求，修改未超出原申请范围。"),
            ("充分公开", "说明书已清楚完整公开，本领域技术人员能够实现。"),
            ("证据不足", "审查员关于公知常识的认定缺乏证据支持，请提供证据。"),
            ("延期请求", "申请人请求延长答复期限以便充分准备意见。"),
        ];
        let found = templates.iter().find(|(n,_)| n.contains(&input.template_type)).map(|(_,t)| t.to_string()).unwrap_or_default();
