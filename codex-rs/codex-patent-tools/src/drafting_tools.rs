...
mod tests {
    ...
    fn specification_draft_contains_all_sections() {
        ...
        let result = specification_draft(input).unwrap();
    ...
    fn claim_generator_with_optional_features() {
        ...
        let result = claim_generator(input).unwrap();
    ...
    fn abstract_draft_contains_components() {
        ...
        let result = abstract_draft(input).unwrap();
    ...
    fn innovation_evaluator_returns_valid_score() {
        let result = innovation_evaluator(
            "一种新型量子计算方法".into(),
            Some("大幅提高计算速度".into()),
            Some(0.8),
            Some(false),
