...
impl StreamableHttpTestServer {
    /// Returns the MCP endpoint URL that BCIP should connect to.
    fn url(&self) -> &str {
        &self.server_url
    }
...
/// What this tests: BCIP can discover and call a Streamable HTTP MCP tool in
...
async fn streamable_http_tool_call_round_trip() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so BCIP will call the MCP echo tool
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so BCIP will call the OAuth-backed
...
async fn start_remote_streamable_http_test_server(
    ...
    // remote HTTP before handing the URL to the BCIP fixture.
