...
#[expect(
    ...
    reason = "BCIP Apps cache refresh reads through the session-owned manager guard"
)]
async fn refresh_codex_apps_after_connector_auth(sess: &Session, turn_context: &TurnContext) {
    ...
    match mcp_tools_result {
        ...
        Err(err) => {
            tracing::warn!("failed to refresh BCIP Apps tools after connector auth: {err:#}");
        }
    }
}
...
async fn augment_mcp_tool_request_meta_with_sandbox_state(
    ...
    let sandbox_state = serde_json::to_value(SandboxState {
        ...
        sandbox_cwd: turn_context.primary_cwd().to_path_buf(),
