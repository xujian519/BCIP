    ...
    if let Err(err) = sess.flush_rollout().await {
        sess.send_event(
            ...
            EventMsg::Warning(WarningEvent {
                message: format!(
                    "Rolled the thread back, but failed to save the rollback marker. BCIP will continue retrying. Error: {err}"
                ),
            }),
        )
        .await;
...
pub async fn review(
    ...
    sess.refresh_mcp_servers_if_requested(&turn_context, Some(sess.mcp_elicitation_reviewer()))
        .await;
    match resolve_review_request(review_request, turn_context.primary_cwd()) {
        Ok(resolved) => {
            spawn_review_thread(
                Arc::clone(sess),
                Arc::clone(config),
