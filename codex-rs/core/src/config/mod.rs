...
// when BCIP is still responsible for the network policy. `Disabled` means no
...
pub struct Config {
    ...
    /// Optional external notifier command. When set, BCIP will spawn this
    ...
    /// file (default): Use a file in the BCIP home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that BCIP can reach out to for tool calls.
    pub mcp_servers: Constrained<HashMap<String, McpServerConfig>>,

    /// Preferred store for MCP OAuth credentials.
    /// keyring: Use an OS-specific keyring service.
    ///          Credentials stored in the keyring will only be readable by BCIP unless the user explicitly grants access via OS-level keyring access.
    ///          https://github.com/xujian519/BCIP/blob/main/codex-rs/rmcp-client/src/oauth.rs#L2
    /// file: CODEX_HOME/.credentials.json
    ///       This file will be readable to BCIP and other applications running as the same user.
    /// auto (default): keyring if available, otherwise file.
    pub mcp_oauth_credentials_store_mode: OAuthCredentialsStoreMode,

    /// Optional fixed port to use for the local HTTP callback server used during MCP OAuth login.
    ///
    /// When unset, BCIP will bind to an ephemeral port chosen by the OS.
    ...
    /// Directory containing all BCIP state (defaults to `~/.codex` but can be
    /// overridden by the `CODEX_HOME` environment variable).
    pub codex_home: AbsolutePathBuf,

    /// Directory where BCIP stores the SQLite state DB.
    pub sqlite_home: PathBuf,

    /// Directory where BCIP writes log files (defaults to `$CODEX_HOME/log`).
    pub log_dir: PathBuf,

    /// Directory where BCIP writes effective session config lock files.
    pub config_lock_export_dir: Option<AbsolutePathBuf>,

    /// Whether config lock replay ignores BCIP version drift between the
    ...
    /// Path to the current BCIP executable. This cannot be set in the config
    ...
    /// When `true`, checks for BCIP updates on startup and surfaces update prompts.
    /// Set to `false` only if your BCIP updates are centrally managed.
    /// Defaults to `true`.
    ...
    /// When `false`, disables analytics across BCIP product surfaces in this machine.
    /// Voluntarily left as Optional because the default value might depend on the client.
    pub analytics_enabled: Option<bool>,

    /// When `false`, disables feedback collection across BCIP product surfaces.
    /// Defaults to `true`.
...
impl Config {
    ...
    /// Load a default configuration for a specific BCIP home without reading
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let (
            ...
            {
                // Keep BCIP runtime write access while storing the runtime
            ...
            && constrained_approval_policy.value() == AskForApproval::Never
        {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "`approval_policy = \"never\"` cannot be used because requirements do not allow `sandbox_mode = \"danger-full-access\"`; BCIP would fall back to read-only permissions with approvals disabled. Choose an `approval_policy` based on what you need, such as `on-request`, or choose an allowed sandbox mode.",
            ));
        }
        if permission_profile_was_constrained {
...
/// Returns the path to the BCIP configuration directory, which can be
/// specified by `BCIP_HOME` or `CODEX_HOME`. If not set, defaults to `~/.bcip`
/// (isolated from the official Codex desktop `~/.codex`).
///
/// - If the env var is set, the value must exist and be a directory. The
///   value will be canonicalized and this function will Err otherwise.
/// - If unset, this function does not verify that the default directory exists.
pub fn find_codex_home() -> std::io::Result<AbsolutePathBuf> {
    codex_utils_home_dir::find_codex_home()
}

/// Returns the path to the folder where BCIP logs are stored. Does not verify
