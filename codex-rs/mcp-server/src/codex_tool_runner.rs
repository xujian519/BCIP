...
/// Run a complete BCIP session and stream events back to the client.
///
...
pub async fn run_codex_tool_session(
    ...
    let NewThread {
        ...
        Err(e) => {
            let result = CallToolResult {
                content: vec![Content::text(format!("Failed to start BCIP session: {e}"))],
                is_error: Some(true),
    ...
    // Use the original MCP request ID as the `sub_id` for the BCIP submission so that
