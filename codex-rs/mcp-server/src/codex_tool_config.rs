...
pub struct CodexToolCallParam {
    /// The *initial user prompt* to start the BCIP conversation.
    pub prompt: String,
...
/// Builds a `Tool` definition (JSON schema etc.) for the BCIP tool-call.
pub(crate) fn create_tool_for_codex_tool_call_param() -> Tool {
    ...
    let input_schema = create_tool_input_schema(schema, "BCIP tool schema should serialize");
    ...
    Tool {
        ...
        description: Some(
            "Run a BCIP session. Accepts configuration parameters matching the BCIP Config struct."
                .into(),
        ),
        annotations: None,
        execution: None,
...
impl CodexToolCallParam {
    /// Returns the initial user prompt to start the BCIP conversation and the
...
pub struct CodexToolCallReplyParam {
    ...
    /// The thread id for this BCIP session.
    ...
    /// The *next user prompt* to continue the BCIP conversation.
    pub prompt: String,
}
...
pub(crate) fn create_tool_for_codex_tool_call_reply_param() -> Tool {
    ...
    Tool {
        ...
        output_schema: Some(codex_tool_output_schema()),
        description: Some(
            "Continue a BCIP conversation by providing the thread id and prompt.".into(),
        ),
        annotations: None,
        execution: None,
        icons: None,
...
mod tests {
    ...
    fn verify_codex_tool_json_schema() {
        ...
        let expected_tool_json = serde_json::json!({
          "description": "Run a BCIP session. Accepts configuration parameters matching the BCIP Config struct.",
          "inputSchema": {
            ...
            "properties": {
              ...
              "prompt": {
                "description": "The *initial user prompt* to start the BCIP conversation.",
                "type": "string"
              },
              "sandbox": {
    ...
    fn verify_codex_tool_reply_json_schema() {
        ...
        let tool_json = serde_json::to_value(&tool).expect("tool serializes");
        let expected_tool_json = serde_json::json!({
          "description": "Continue a BCIP conversation by providing the thread id and prompt.",
          "inputSchema": {
            "properties": {
              "conversationId": {
              ...
              "prompt": {
                "description": "The *next user prompt* to continue the BCIP conversation.",
                "type": "string"
              },
              "threadId": {
                "description": "The thread id for this BCIP session. This field is required, but we keep it optional here for backward compatibility for clients that still use conversationId.",
                "type": "string"
              }
            },
            "required": [
