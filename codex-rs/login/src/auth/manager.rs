...
impl CodexAuth {
    ...
    /// Returns `None` if BCIP backend auth does not expose an account id.
    ...
    /// Returns false if BCIP backend auth omits the FedRAMP claim.
    ...
    /// Returns `None` if BCIP backend auth does not expose an account email.
    ...
    /// Returns `None` if BCIP backend auth does not expose a ChatGPT user id.
...
pub trait AuthManagerConfig {
    /// Returns the BCIP home directory used for auth storage.
