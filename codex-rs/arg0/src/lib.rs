...
pub struct Arg0DispatchPaths {
    /// Stable path to the current BCIP executable for child re-execs.
    ///
    ...
    /// instead of the real YunPat Agent.
...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    match prepend_path_entry_for_codex_aliases() {
        ...
        Err(err) => {
            // It is possible that BCIP will proceed successfully even if
...
/// While we want to deploy the YunPat Agent as a single executable for simplicity,
