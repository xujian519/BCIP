...
`codex-app-server-daemon` backs the machine-readable `bcip app-server`
lifecycle commands used by remote clients such as the desktop and mobile apps.
It is intended for BCIP Agent instances launched over SSH, including fresh developer
...
```sh
bcip app-server daemon start
bcip app-server daemon restart
bcip app-server daemon enable-remote-control
bcip app-server daemon disable-remote-control
bcip app-server daemon stop
bcip app-server daemon version
bcip app-server daemon bootstrap --remote-control
```
...
The daemon assumes BCIP Agent is installed through `install.sh` and always launches
