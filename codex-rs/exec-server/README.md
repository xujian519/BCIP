...
`codex-exec-server` is the library backing `bcip exec-server`, a small
JSON-RPC server for spawning and controlling subprocesses through
`codex-utils-pty`.

It provides:

- a CLI entrypoint: `bcip exec-server`
...
`bcip exec-server` command builds these paths from the `codex` arg0 dispatch
