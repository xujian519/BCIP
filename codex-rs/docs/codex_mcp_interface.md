# BCIP MCP Server Interface [experimental]

This document describes BCIP's experimental MCP server interface: a JSON-RPC API that runs over the Model Context Protocol (MCP) transport to control a local BCIP engine.

- Status: experimental and subject to change without notice
- Server binary: `bcip mcp-server` (or `codex-mcp-server`)
- Transport: standard MCP over stdio (JSON-RPC 2.0, line-delimited)

## Overview

BCIP exposes MCP-compatible methods to manage threads, turns, accounts, config, and approvals. The types live in `app-server-protocol/src/protocol/{common,v1,v2}.rs` and are consumed by the app server implementation in `app-server/`.
...
```bash
bcip mcp-server | your_mcp_client
```
...
- `codex/event` with the serialized BCIP event payload. The shape matches `core/src/protocol.rs`'s `Event` and `EventMsg` types. Some notifications include a `_meta.requestId` to correlate with the originating request.
