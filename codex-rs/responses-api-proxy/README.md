...
bcip exec -c "model_providers.openai-proxy={ name = 'OpenAI Proxy', base_url = '${PROXY_BASE_URL}/v1', wire_api='responses' }" \
...
- Authentication is fixed to `Authorization: Bearer <key>` to match the BCIP Agent expectations.
...
- We leverage [`codex_process_hardening`](https://github.com/xujian519/BCIP/blob/main/codex-rs/process-hardening/README.md) so `codex-responses-api-proxy` is run with standard process-hardening techniques.
