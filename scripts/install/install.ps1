...
function Find-ReleaseAssetMetadata {
    ...
    $release = Invoke-RestMethod -Uri "https://api.github.com/repos/xujian519/BCIP/releases/tags/rust-v$ResolvedVersion"
...
function Resolve-Version {
    ...
    $release = Invoke-RestMethod -Uri "https://api.github.com/repos/xujian519/BCIP/releases/latest"
...
function Maybe-HandleConflictingInstall {
    ...
    $uninstallArgs = if ($manager -eq "bun") {
        @("remove", "-g", "@xujian519/bcip-agent")
    } else {
        @("uninstall", "-g", "@xujian519/bcip-agent")
    }
...
if (-not [string]::IsNullOrWhiteSpace($currentVersion) -and $currentVersion -ne $resolvedVersion) {
    Write-Step "Updating BCIP Agent from $currentVersion to $resolvedVersion"
} elseif (-not [string]::IsNullOrWhiteSpace($currentVersion)) {
    Write-Step "Updating BCIP Agent"
} else {
    Write-Step "Installing BCIP Agent"
}
...
try {
    Invoke-WithInstallLock -LockPath $lockPath -Script {
        ...
        if (-not (Test-ReleaseIsComplete -ReleaseDir $releaseDir -ExpectedVersion $resolvedVersion -ExpectedTarget $target -Layout $installLayout)) {
            ...
            Write-Step "Downloading BCIP Agent"
...
Write-Host "BCIP Agent $resolvedVersion installed successfully."
