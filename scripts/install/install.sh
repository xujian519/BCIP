...
release_url_for_asset() {
  ...
  printf 'https://github.com/xujian519/BCIP/releases/download/rust-v%s/%s\n' "$resolved_version" "$asset"
}

release_metadata_url() {
  resolved_version="$1"

  printf 'https://api.github.com/repos/xujian519/BCIP/releases/tags/rust-v%s\n' "$resolved_version"
}
...
file_sha256() {
  ...
  echo "sha256sum, shasum, or openssl is required to verify the BCIP download." >&2
  exit 1
}
...
resolve_version() {
  ...
  release_json="$(download_text "https://api.github.com/repos/xujian519/BCIP/releases/latest")"
...
handle_conflicting_install() {
  ...
  case "$conflict_manager" in
    brew)
      uninstall_cmd="brew uninstall --cask bcip-agent"
      ;;
    bun)
      uninstall_cmd="bun remove -g @xujian519/bcip-agent"
      ;;
    *)
      uninstall_cmd="npm uninstall -g @xujian519/bcip-agent"
      ;;
  esac
...
if [ -n "$current_version" ] && [ "$current_version" != "$resolved_version" ]; then
  step "Updating BCIP Agent from $current_version to $resolved_version"
elif [ -n "$current_version" ]; then
  step "Updating BCIP Agent"
else
  step "Installing BCIP Agent"
fi
...
printf 'BCIP Agent %s installed successfully.\n' "$resolved_version"
maybe_launch_codex_now
