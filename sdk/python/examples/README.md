...
uses the installed `bcip-agent-cli-bin` runtime package.

If the pinned `bcip-agent-cli-bin` runtime is not already installed, the bootstrap
will download the matching GitHub release artifact, stage a temporary local
`bcip-agent-cli-bin` package, install it into your active interpreter, and clean up
...
active interpreter and an installed `bcip-agent-cli-bin` runtime package (either
