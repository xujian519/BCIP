# OpenAI BCIP Agent Python SDK (Experimental)

Experimental Python SDK for `bcip app-server` JSON-RPC v2 over stdio, with a small default surface optimized for real scripts and apps.

The generated wire-model layer is sourced from the pinned `bcip-agent-cli-bin`
...
Published SDK builds pin an exact `bcip-agent-cli-bin` runtime dependency
...
Published SDK builds are pinned to an exact `bcip-agent-cli-bin` package
...
- Package: `bcip-agent`
- Runtime package: `bcip-agent-cli-bin`
- Python: `>=3.10`
- Target protocol: BCIP Agent `app-server` JSON-RPC v2
- Versioning rule: the SDK package version is the underlying BCIP Agent runtime version

## Notes

- `BCIP Agent()` is eager and performs startup + `initialize` in the constructor.
