...
const CODEX_NPM_NAME = "@xujian519/bcip-agent";

const PLATFORM_PACKAGE_BY_TARGET: Record<string, string> = {
  "x86_64-unknown-linux-musl": "@xujian519/bcip-agent-linux-x64",
  "aarch64-unknown-linux-musl": "@xujian519/bcip-agent-linux-arm64",
  "x86_64-apple-darwin": "@xujian519/bcip-agent-darwin-x64",
  "aarch64-apple-darwin": "@xujian519/bcip-agent-darwin-arm64",
  "x86_64-pc-windows-msvc": "@xujian519/bcip-agent-win32-x64",
  "aarch64-pc-windows-msvc": "@xujian519/bcip-agent-win32-arm64",
};
  ...
  async *run(args: CodexExecArgs): AsyncGenerator<string> {
    ...
    try {
      ...
      if (code !== 0 || signal) {
        ...
        throw new Error(`BCIP Agent Exec exited with ${detail}: ${stderrBuffer.toString("utf8")}`);
      }
    } finally {
      rl.close();
...
function flattenConfigOverrides(
  ...
  if (!isPlainObject(value)) {
    if (prefix) {
      ...
      throw new Error("BCIP config overrides must be a plain object");
    }
  }
  ...
  for (const [key, child] of entries) {
    if (!key) {
      throw new Error("BCIP config override keys must be non-empty strings");
    }
    if (child === undefined) {
      continue;
    }
...
function toTomlValue(value: CodexConfigValue, path: string): string {
  if (typeof value === "string") {
    ...
    if (!Number.isFinite(value)) {
      throw new Error(`BCIP config override at ${path} must be a finite number`);
    }
    return `${value}`;
    ...
    throw new Error(`BCIP config override at ${path} cannot be null`);
  } else {
    const typeName = typeof value;
    throw new Error(`Unsupported BCIP config override value at ${path}: ${typeName}`);
  }
}
...
function findCodexPath(): CodexPathResolution {
  ...
  try {
    ...
    throw new Error(
      `Unable to locate BCIP Agent CLI binaries. Ensure ${CODEX_NPM_NAME} is installed with optional dependencies.`,
    );
  }

  const codexBinaryName = process.platform === "win32" ? "bcip-agent.exe" : "bcip-agent";
  const nativePackage = resolveNativePackage(vendorRoot, targetTriple, codexBinaryName);
  if (!nativePackage) {
    throw new Error(
      `Unable to locate BCIP Agent CLI binaries for ${targetTriple}. Ensure ${CODEX_NPM_NAME} is installed with optional dependencies.`,
    );
  }
